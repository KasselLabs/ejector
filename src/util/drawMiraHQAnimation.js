...
function drawCharacter (canvas, context, characterImages, elapsed) {
  const ACCELERATION_Y = 0.13
  const SPEED_Y = 0 + (ACCELERATION_Y * elapsed)
  const INITIAL_ROTATION = -0.1
  const ROTATION_SPEED = 0.63

  const positionX = (canvas.width / 2)
  const positionY = canvas.height * (SPEED_Y * elapsed) - (canvas.height / 10)
  ...
  const height = canvas.height / 5.7
  const width = height * aspectRatio

  context.save()
  context.translate(positionX, positionY)
  context.rotate(INITIAL_ROTATION - (elapsed * ROTATION_SPEED))
  context.drawImage(
    characterImage,
    -width / 2,
    -height / 2,
...
const drawMiraHQAnimation = async (canvas, ejectedText, impostorText, characterImages, elapsed, showWatermark = true) => {
...
export default drawMiraHQAnimation
